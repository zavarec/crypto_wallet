...
import { addToFavorites, deleteFavoriteCoin, getCoins } from '../../services/apiCoinsMarket';

const url = import.meta.env.VITE_URL_API_COINRANKING as string;

export const getCoinsThunkAction = createAsyncThunk<DataStateType>('coins/fetchCoins', async () =>
  getCoins(),
);

export const addToFavoritesThunkAction = createAsyncThunk<
  Promise<FavoriteCoin['ticket_name']>,
  string
>('coins/addToFavorites', async (coinId) => addToFavorites(coinId));
